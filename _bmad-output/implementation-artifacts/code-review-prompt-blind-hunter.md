...
- Do not assume any project context, coding standards, or prior architecture.
- Review adversarially for defects, regressions, and suspicious changes.

Output format:
- Return findings as a Markdown list.
- Each finding must include: one-line title, severity (high/medium/low), and concise evidence from the diff.
